...
/*****************************************************************/
void get_pi_typed_survey (int *type,
		   double *x,
		   double *y,
		   int *len,
		   int *typeA,
  ...
  for (i=0;i<*len_r;i++) {
	...
	for (k=0;k<*len;k++) {
      ...
      /* ignore pairs with sampled infected*/
	  if (type[k] == *typeA) continue;
...
/*@param rc      the array of values that we are going to return    */
/********************************************************************/
void get_theta_typed_survey (int *type,
		   double *x,
		   double *y,
		   int *len,
		   int *typeA,
...
/*@param rc      the array of values that we are going to return */
/*****************************************************************/
void get_tau_typed_survey (int *type,
		    double *x,
		    double *y,
		    int *len,
		    int *typeA,
    ...
    if (*comp_type==0) {
      ...
      get_pi_typed_survey(type,x,y,len,typeA,typeB,&tmp_r_low, &tmp_r,
		   &tmp_len_r,inds,&divisor);

      /*get the main pi function*/
      get_pi_typed_survey(type, x, y, len, typeA, typeB, r_low, r, len_r,inds, rc);
    } else if (*comp_type==1) {
      /*get the divisor in the pi function*/
      get_theta_typed_survey(type,x,y,len,typeA,typeB,&tmp_r_low, &tmp_r,
		   &tmp_len_r,inds,&divisor);

      /*get the main pi function*/
      get_theta_typed_survey(type, x, y, len, typeA, typeB, r_low, r, len_r,inds, rc);
    }
